  ...
  const fetchProducts = async (pageCount = 1) => {
    try {
      const { data } = await axios.get(
        `/api/admin/vendors/products?count=${pageCount}&type=unpublished`
      );
      if (data.success) {
  ...
  return (
    <>
      <div className="page-header">
        ...
        <p>Products awaiting review and approval</p>
      ...
      {products.length > 0 ? (
          ...
          <TableBody>
            {products.map((product: any) => (
              <TableRow key={product.productId}>
                ...
                <TableCell>
                  <div>
                    <p className="font-bold">{product.vendorId?.name}</p>
        ...
        <div className="empty-state">
          ...
          <h3>No unpublished products found</h3>
          <p>There are no products awaiting review.</p>
        </div>
      )}
