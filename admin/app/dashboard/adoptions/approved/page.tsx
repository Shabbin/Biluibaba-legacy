...
export default function Page() {
  ...
  const fetchAdoptions = async (pageCount = 0) => {
    try {
      const { data } = await axios.get(
        `/api/admin/adoptions/fetch?count=${pageCount}&status=approved`
      );
  ...
  return (
    <div className="py-5">
      <h2 className="text-4xl">Approved Adoptions</h2>
      ...
      {loading ? (
        ...
        <>
          <Table className="my-5">
            <TableCaption>List of all approved adoptions</TableCaption>
            <TableHeader>
              <TableRow>
