...
export default function Page() {
  ...
  const fetchImages = async () => {
    try {
      ...
      if (data.success && data.site.product_landing_slider.length > 0)
        setImages(data.site.product_landing_slider);
    } catch (error) {
      console.error(error);
      toast({
        title: "Server Error",
  ...
  const removeImage = async (image: string | null) => {
    ...
    try {
      const { data } = await axios.delete(
        `/api/admin/site-settings/product-landing-slider/${image}`
      );
      ...
      if (data.success) {
        ...
        return setImages(data.site.product_landing_slider);
      }
    } catch (error) {
      console.error(error);
      toast({
  ...
  return (
    <div className="py-5">
      <h2 className="text-3xl">Product Landing Slider</h2>
      <p className="mt-3 text-lg">
        This is the image slider which comes up when the site first loads.
      </p>
