...
const Product: React.FC<ProductProps> = ({
  ...
  const handleClick = (): void => {
    ...
    if (wishlistItems.some((item) => item.slug === slug)) {
      const updatedWishlist = wishlistItems.filter((item) => item.slug !== slug);
  ...
  const calculatedPrice = Math.floor(price - (discount > 0 ? (price * discount) / 100 : 0));
        ...
        <div>
          ...
          {discount > 0 && (
            <div className="flex flex-row items-center gap-2 mb-4">
              <div className="text-xl text-gray-400 line-through">&#2547; {price}</div>
