...
interface StepFiveProps {
  ...
  handleSpecializedZoneChange: (
    index: number,
    event: { target: { name: string; value: string | ConcernOption[] } }
  ) => void;
...
interface StepSevenProps extends StepProps {
...
const Steps: Step[] = [
  ...
  { id: 5, name: "Specialized Zone" },
  { id: 6, name: "Bank Details" },
  { id: 7, name: "Create Password and Continue" },
];
...
export default function Page(): JSX.Element {
  ...
  const [vetData, setVetData] = useState<VetData>({
    ...
    hospital: "",
    tin: "",
    specializedZone: [{ pet: "Dog", concerns: [] }],
  ...
  const handleSpecializedZoneChange = (
    index: number,
    event: { target: { name: string; value: string | ConcernOption[] } }
  ): void => {
    const { name, value } = event.target;
    ...
    (specializedZone[index] as Record<string, string | ConcernOption[]>)[name] = value;

    setVetData({ ...vetData, specializedZone });
  };

  const handleSpecializedZoneAddFields = (): void => {
  ...
  const handleVetFileChange = (event: ChangeEvent<HTMLInputElement>): void => {
            ...
            <div className="basis-full">
              ...
              {step === 5 && (
                <StepFive
                  ...
                  handleSpecializedZoneAddFields={handleSpecializedZoneAddFields}
                  handleSpecializedZoneRemoveFields={handleSpecializedZoneRemoveFields}
                  setStep={setStep}
                />
              )}
...
function StepOne({ data, handleDataChange, setStep }: StepProps): JSX.Element {
  ...
  const checkVetEmail = async (): Promise<void> => {
    if (!data.name || data.name === "")
      ...
      return void toast.error("Please provide a valid gender.");
...
function StepTwo({
  ...
  const checkDocuments = (): void => {
    if (!data.nidFront || data.nidFront === null)
      ...
      return void toast.error("Please provide an image of your certificate.");
    else if (!data.degree || data.degree === "")
      return void toast.error("Please provide your degree name.");
    else if (!data.license || data.license === "")
      return void toast.error("Please provide your license number.");
    else setStep(3);
  };
...
function StepThree({ data, handleDataChange, setStep }: StepProps): JSX.Element {
  const checkAddress = (): void => {
    if (!data.hospital || data.hospital === "")
      return void toast.error("Please provide your hospital name.");
    else if (!data.fullAddress || data.fullAddress === "")
      return void toast.error("Please provide your store address.");
    else if (!data.state || data.state === "")
      return void toast.error("Please provide your store division.");
    else if (!data.district || data.district === "")
      return void toast.error("Please provide your store district.");
    else if (!data.postcode || data.postcode === "")
      return void toast.error("Please provide your store postcode.");
    else setStep(4);
  };
  ...
  return (
    <div className="my-5 flex flex-row items-center justify-between gap-y-2 flex-wrap">
      ...
      <div className="basis-1/2 -m-2 p-2">
        <label htmlFor="fullAddress">Hospital address</label>
        <Input
          type="text"
          name="fullAddress"
...
function StepFive({
  ...
  setStep,
}: StepFiveProps): JSX.Element {
  const checkSpecializedZone = (): void => {
    if (data.specializedZone.length === 0)
      return void toast.error("Please add at least one specialized zone");
    setStep(6);
  };
        ...
        <div key={index} className="basis-full">
          ...
          <div className="basis-full">
            ...
            <Select
              ...
              onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                handleSpecializedZoneChange(index, event)
              }
              name="pet"
            />
          </div>
          <div className="basis-full">
            ...
            <MultiSelect
              ...
              isMulti
              onChange={(value: MultiValue<ConcernOption>) =>
                handleSpecializedZoneChange(index, {
                  target: { value: value as ConcernOption[], name: "concerns" },
                })
              }
            />
          </div>
...
function StepSix({ data, handleDataChange, setStep }: StepProps): JSX.Element {
